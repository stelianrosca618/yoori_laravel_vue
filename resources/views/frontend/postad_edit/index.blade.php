...
@section('content')
<x-frontend.breadcrumb.breadcrumb :links="[['text' => 'Dashboard', 'url' => '/dashboard'], ['text' => 'Edit Post']]" />
<section class="py-16">
    <div class="container">
        <div class="mx-auto" style="max-width: 872px;">
            <div class="flex flex-wrap gap-4 justify-between items-center mb-6">
                <h2 class="sm:heading-01 heading-03 dark:text-white text-gray-900">{{ __('edit_listing') }}</h2>
            ...
            <div class="dark:bg-white rounded-xl border border-gray-100 shadow-[0px_4px_8px_0px_rgba(28,33,38,0.08)]">
                <div class="post-header">
                    <button type="button" class="btn-tab cursor-not-allowed {{ request()->routeIs('frontend.post.edit') ? 'active':'' }}" >
                        <x-frontend.icons.adjustment-horizontal class="w-4 h-4 sm:w-6 sm:h-6" />
                        <span>{{ __('basic') }}</span>
                    </button>
                    <button type="button" class="btn-tab cursor-not-allowed {{ request()->routeIs('frontend.post.edit.step2') ? 'active':'' }}">
                        <x-frontend.icons.adjustment-horizontal class="w-4 h-4 sm:w-6 sm:h-6" />
                        <span>{{ __('detail_info') }}</span>
                    </button>
                    <button type="button" class="btn-tab cursor-not-allowed {{ request()->routeIs('frontend.post.edit.step3') ? 'active':'' }}">
                        <x-frontend.icons.adjustment-horizontal class="w-4 h-4 sm:w-6 sm:h-6" />
                        <span>{{ __('contact_and_promote') }}</span>
                    </button>
                </div>
...
@push('js')
    <script>
        ...
        function updateCancelEdit() {
            document.getElementById('cancel_edit_input').value = 1
            document.getElementById('step_edit_form').submit()
        }
    </script>
@endpush
