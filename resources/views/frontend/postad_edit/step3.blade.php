...
@section('post-ad-content')
    <div x-data="{ whatsapp: {{ $ad->whatsapp }} ? 'true':'false'}">
        <form action="{{ route('frontend.post.step3.update', $ad->slug) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="sm:p-8 p-4 flex flex-col gap-8">
                <div class="flex flex-col gap-4">
                    <div>
                        <div class="w-full flex flex-wrap gap-4 justify-between items-center mb-1.5">
                            ...
                            <div>
                                <div class="tc-checkbox">
                                    <input @change="whatsapp = !whatsapp" type="checkbox" name="" id="whatsapp" @checked($ad->whatsapp)>
                                    <label for="whatsapp">
                        ...
                        <div>
                            <input required type="text" placeholder="{{ __('phone') }}" class="tc-input @error('phone') @twMerge('focus:border-red-500 hover:border-red-500 border-red-500') @enderror" name="phone" value="{{ old('phone', $ad?->phone) }}" id="phone">
                    ...
                    <div x-show="whatsapp" x-cloak>
                        ...
                        <input type="text" placeholder="{{ __('whatsapp_number') }}" class="tc-input" :name="whatsapp ? 'whatsapp':''" value="{{ old('whatsapp', $ad?->whatsapp) }}">
                    </div>
                    <div>
                        <label for="email" class="tc-label">{{__('email_address')}}</label>
                        <input type="email" name="email" id="email" class="tc-input" value="{{ old('email', $ad?->email ?? auth()->user()->email) }}" placeholder="{{ __('email_address') }}">
                    </div>
                </div>
                ...
                <div class="promotions-wrapper">
                    ...
                    <div class="flex items-center mb-2">
                        ...
                        <p class="text-sm">
                            ({{ __('know_more_about') }} <a href="{{ route('frontend.promotions') }}" class="pl-1 underline text-blue-500" target="_blank">{{ __('promotions') }}</a>):
                        </p>
                    </div>
            ...
            <div class="post-footer">
                <button class="btn-secondary py-3 px-5">{{ __('save_on_draft') }}</button>
                <div>
                    <a href="{{ route('frontend.post.step2.back') }}" onclick="return confirm('{{ __('are_you_sure_to_go_back') }}')" class="py-3 px-5">{{ __('back') }}</a>
                    @if ($ad->resubmission==1)
                        <button type="submit" class="btn-primary py-3 px-5">{{ __('save_and_resubmit') }}</button>
                    @else
                        <button type="submit" class="btn-primary py-3 px-5">{{ __('save_and_finish') }}</button>
                    @endif
            </div>
        </form>
    </div>
