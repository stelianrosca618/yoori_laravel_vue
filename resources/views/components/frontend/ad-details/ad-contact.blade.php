...
<div class="product-item__sidebar-item">
    ...
    @if (auth('user')->check() && auth('user')->user()->username !== $ad->customer->username)
        <form action="{{ route('frontend.message.store', $ad->customer->username) }}" method="POST"
            ...
            <button type="submit" class="btn w-100">
                <span class="icon--left">
                    <x-svg.message-icon width="24" height="24" stroke="currentColor" strokeWidth="1.6" />
                </span>
                {{ __('send_message') }}
            </button>
        </form>
        @if ($ad->whatsapp)
            <a href="https://wa.me/{{ $ad->whatsapp }}" class="btn w-100 mt-2 bg-success text-white" target="_blank">
                <span class="icon--left">
                    <x-svg.whatsapp-icon />
                </span>
        ...
        @if ($ad->customer->email)
            <a href="mailto:{{ $ad->customer->email }}" class="btn w-100 mt-2 bg-secondary text-white">
                <span class="icon--left">
    ...
    @endif
    @if (!auth('user')->check())
        <a href="{{ route('users.login') }}" class="btn w-100 login_required text-white">
            <span class="icon--left">
    ...
    @endif
    <div style="color: #E2153D; font-size: 14px; margin-top: 12px; display: flex; flex-direction: column; gap: 8px;">
        <p>{{$cms->ads_safety_msg}}</p>
