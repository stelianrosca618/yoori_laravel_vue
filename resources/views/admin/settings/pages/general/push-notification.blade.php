...
@section('general-setting')
    <div class="card">
        <form class="form-horizontal" action="{{ route('settings.general.push-notification.update') }}" method="POST">
            @method('PUT')
            @csrf
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        <input type="checkbox" name="push_notification_status" data-on-color="success" class="form-control"
                            data-on-text="{{ __('on') }}" data-off-color="default" data-off-text="{{ __('off') }}"
                            data-bootstrap-switch value="1" {{ $setting->push_notification_status ? 'checked' : '' }}>
                    </div>
                </div>
