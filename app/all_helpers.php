...
use App\View\Components\Backend\Setting\Cms\Ads;
use Carbon\Carbon;
...
// if (! function_exists('uploadBase64FileToPublic')) {
...
if (! function_exists('cms')) {
    function cms($fields = null, $append = false)
    {
        if ($fields) {
            ...
            if ($type == 'string') {
                $data = $append ? Cms::first($fields) : Cms::value($fields);
            } elseif ($type == 'array') {
                $data = Cms::first($fields);
            }
        } else {
            $data = 'no data found!';
        }
...
if (! function_exists('advertisementCode')) {
    function advertisementCode($page_slug)
    {
        ...
        $code = '';
        $ad = $ads->where('page_slug', $page_slug)->first();
        if ($ad) {
            $code = $ad->ad_code;
        }
...
//     function minmax()
//     {
//         $data['maxPrice'] =  Ad::max('price');
//         $data['minPrice'] =  Ad::min('price');
